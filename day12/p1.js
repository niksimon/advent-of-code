...
class Position {
    ...
    setStatus(currentPos, newPos, grid) {
        ...
        if (
            ...
            newPos.left === endPoint.left &&
            newPos.top === endPoint.top &&
            grid[newPos.top][newPos.left].char - 1 <=
                grid[currentPos.top][currentPos.left].char
        ) {
            this.status = "End";
        } else if (
            grid[newPos.top][newPos.left].visited ||
            grid[newPos.top][newPos.left].char >
                grid[currentPos.top][currentPos.left].char + 1
        ) {
            this.status = "Blocked";
        } else {
            this.status = "Valid";
...
console.log(findShortestPath(startPoint, grid).length);
